...
class Window2:
    ...
    def _create_widgets(self):
        ...
        # 選択されたファイル表示
        self.config_file_var = tk.StringVar(value="設定ファイルが選択されていません")
        config_label = ttk.Label(file_frame, textvariable=self.config_file_var, foreground="gray")
    ...
    def _select_config_file(self):
        ...
        if file_path:
            ...
            self._add_log(f"設定ファイルが選択されました: {os.path.basename(file_path)}")
        else:
            self._add_log("設定ファイルの選択がキャンセルされました。")
    ...
    def _run_pipeline2(self):
        ...
        if not hasattr(self, 'config_file_path'):
            ...
            self._add_log("エラー: 設定ファイルが選択されていません")
            return
        
        if not hasattr(self, 'output_folder_path'):
            messagebox.showwarning("警告", "出力フォルダを選択してください")
            self._add_log("エラー: 出力フォルダが選択されていません")
            return
        
        # 実行ログ
        self._add_log("=" * 50)
        self._add_log("パイプライン2の実行を開始します...")
        self._add_log(f"設定ファイル: {self.config_file_path}")
        self._add_log(f"出力フォルダ: {self.output_folder_path}")
        self._add_log(f"デバッグモード: {'有効' if self.debug_mode.get() else '無効'}")
        self._add_log(f"バックアップ作成: {'有効' if self.backup_mode.get() else '無効'}")
        self._add_log("-" * 30)
...
class FileManagerApp:
    ...
    def _create_widgets(self):
        ...
        result_frame.grid(row=1, column=0, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(10, 0))
    ...
    def _run_pipeline1(self):
        ...
        try:
            ...
            self.result_text.see(tk.END)
            messagebox.showerror("エラー", f"パイプライン1の実行に失敗しました:\n{str(e)}")
